...
@ViewEntity({name:'dfg_report_saleszm',
  expression: `SELECT --TOP 1 
			...
			sb.cDefine34,
			--i.cInvCCode,
			CASE WHEN po.iNatUnitPrice IS NOT NULL THEN '有采'
			     WHEN po.iNatUnitPrice IS NULL THEN '无采'
            END AS cg,
			po.iNatUnitPrice,
			ISNULL(po.iNatUnitPrice,ISNULL(up.outiunitprice, 0))AS unn,
			sb.iQuantity * ISNULL(po.iNatUnitPrice,ISNULL(up.outiunitprice, 0))AS cgPrice
		...
		FROM        dbo.SaleBillVouchs AS sb
                    ...
                    LEFT JOIN dbo.PO_Podetails AS po ON sd.cDefine34 = po.cDefine34 
	--			                                        AND sd.cInvCode = po.cInvCode
			WHERE   sb.iDLsID IS  NOT NULL
			         AND LEFT(i.cInvCCode, 3) IN ( '302', '305' )
                     AND i.bPurchase = 1
			        
			--AND ISNULL(stk.rdateDl,ISNULL(stk.rdateBsc,ISNULL(stk.ZRdate,ISNULL(stk.rdateKh,NULL)))) IS NULL`,
})
export class SalesZm extends BaseEntity {
  @ViewColumn()
