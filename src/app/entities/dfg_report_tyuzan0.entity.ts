...
@ViewEntity({
  expression: `SELECT TOP 100 PERCENT
        ...
        i.cInvCCode ,
        sm.dDate ,
		DATEDIFF(wk,sm.dDate,GETDATE())AS diffdd ,
		DATEDIFF(wk,so.dpredate,GETDATE())AS diffDp,
		DATEDIFF(wk,sm.ddate,so.dPreDate) AS diffpd,
