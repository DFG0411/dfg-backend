...
@ViewEntity({name:'dfg_report_po',
  expression: `SELECT  pd.ID ,
			...
			ip.cInvCCode AS pinvccode,
			ip.cInvCode AS pinvcode,
			ip.cInvStd AS pinvstd,
			ip.cInvAddCode AS pinvaddcode
    FROM    dbo.PO_Podetails AS pd
			...
			LEFT JOIN dbo.Inventory AS ip ON ip.cInvCode = sd.cInvCode
...
export class RptPo extends BaseEntity{
  @ViewColumn()
  id: number;
  @ViewColumn()
  cCode: string | null;
  ...
  pdArriveDate: Date;
  @ViewColumn()
  pinvccode: string | null;
  @ViewColumn()
  pinvcode: string | null;
  @ViewColumn()
  pinvstd: string | null;
  @ViewColumn()
  pinvaddcode: string | null;
}
